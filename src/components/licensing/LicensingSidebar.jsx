...
export default function LicensingSidebar({ user, shortName, activeTab, onTabChange }) {
  ...
  return (
    <aside className="hidden md:flex flex-col items-center w-24 shrink-0 bg-white border-r border-gray-200 py-5 gap-1 sticky top-0 h-screen overflow-y-auto">
      ...
      <p className="text-[10px] font-medium text-gray-500 text-center leading-tight mb-3 px-1 truncate w-full">{shortName}</p>

      <nav className="flex flex-col gap-3 w-full px-2 pb-6">
        {grupos.map((grupo) => (
          <div key={grupo.title} className="flex flex-col gap-1">
            <p className="text-[8px] font-bold uppercase tracking-wider text-nz-marrom/60 text-center">{grupo.title}</p>
