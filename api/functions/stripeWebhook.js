...
export default async function handler(req, res) {
  try {
    ...
    // 💰 Sem kind específico → mesmo motor oficial das vendas 'loja' (arvoreOficial 30%),
    // igual ao mpWebhook. Fecha a brecha de um motor de comissão diferente por gateway.
    const r = await fulfillStoreOrder(sale);
    return res.status(200).json({ ok: true, paid: true, sale_id: sale.id, ...r });
  } catch (e) {
