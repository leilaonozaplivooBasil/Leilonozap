...
export default function EmCenaAgora({ produto, bidAmount, setBidAmount, onBid, isSubmitting, logado }) {
  if (!produto) {
    return (
      <Card className="rounded-xl border-livoo-rosa/25 bg-white/5 p-8 text-center">
        ...
        <p className="text-white/60">O próximo leilão já já começa.</p>
      </Card>
    );
  }
  ...
  return (
    <Card className="overflow-hidden rounded-xl border-livoo-rosa/25 bg-white/5">
      <div className="flex items-center justify-between bg-livoo-rosa px-4 py-2.5">
      ...
      <div className="p-5">
        ...
        <div className="mt-4 text-center">
          ...
          <p className="mt-1 text-[11px] text-white/50">Incremento mínimo: + R$ {fmtBR(produto.increment)}</p>
        </div>
          ...
          {[1, 2, 5].map((mult) => {
            ...
            return (
              <Button
                ...
                className="h-auto flex-col bg-nz-verde py-3 text-white hover:bg-nz-verde-claro"
              >
        ...
        <div className="mt-3 flex gap-2">
          ...
          <Button
            ...
            className="bg-nz-verde px-6 text-white hover:bg-nz-verde-claro"
          >
            {isSubmitting ? "..." : "Dar Lance"}
          </Button>
        </div>
        {!logado && <p className="mt-2 text-center text-xs text-white/50">Faça login para participar</p>}
      </div>
    </Card>
  );
}