...
Deno.serve(async (req) => {
  try {
    ...
    // Aceita seller_id direto no body (vendedor customizado, sem auth Base44)
    // OU tenta via base44.auth.me() para admin
    let sellerId = body.seller_id || null;

    if (!sellerId) {
      // Tenta auth Base44 (admin)
      try {
        ...
        if (caller?.email) {
          ...
          if (callerUser) {
            if (callerUser.role === 'admin' && body.seller_id) {
              sellerId = body.seller_id;
            } else {
              sellerId = callerUser.id;
            }
          }
        }
      } catch (e) { /* sem auth Base44 — vendedor customizado */ }
    }
    ...
    // Buscar vendas (CatalogSale onde licensee_id === sellerId)
    ...
    const withdrawals = await base44.asServiceRole.entities.WithdrawalRequest.filter(
      { influencer_id: sellerId }, '-created_date', 100
    );
    ...
    const ultimas10 = ultimasVendas
      .sort((a, b) => new Date(b.payment_confirmed_date) - new Date(a.payment_confirmed_date))
      .slice(0, 10);
    ...
    return Response.json({
      ...
      saques_recentes: safeWithdrawals.slice(0, 10),
