...
import { DollarSign, Zap, BarChart, ShieldCheck, LogIn, Smartphone, Star } from 'lucide-react';
...
export default function LandingContent({ onRegisterClick, onLoginClick }) {
  ...
  const isSDB = sessionStorage.getItem('saiDeBaixoContext') === 'true';
      ...
      <div className="text-center">
        <div className="mb-12">
          <div className="inline-flex flex-col items-center gap-3 bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 border-2 border-gray-700 hover:border-green-500/50 transition-all duration-300">
            ...
            <button onClick={onLoginClick} className={`px-8 py-4 bg-gradient-to-r text-white font-bold text-lg rounded-xl shadow-lg transition-all duration-300 transform hover:scale-105 flex items-center gap-3 ${isSDB ? 'from-red-500 to-red-600 hover:from-red-600 hover:to-red-700' : 'from-green-500 to-green-600 hover:from-green-600 hover:to-green-700'}`}>
        ...
        <div className="relative mb-8">
          ...
          <div className="relative flex justify-center">
            <button onClick={() => cardsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="px-4 bg-gray-900 text-gray-500 text-sm font-medium hover:text-green-400 transition-colors cursor-pointer">Ou cadastre-se agora</button>
          </div>
        </div>
      </div>
      ...
      <div ref={cardsRef} className="mt-16 max-w-2xl mx-auto">
        <Card className="bg-gray-800/80 backdrop-blur-sm border-2 border-green-500/50 shadow-xl hover:shadow-green-500/30 hover:border-green-400 transition-all duration-300">
          ...
          <CardContent className="space-y-6">
            <p className="text-gray-200 text-base leading-relaxed">Torne-se um <strong className="text-white">Influenciador Leilão NoZap</strong> e receba <strong className="text-green-400">3% em dinheiro real (R$)</strong> sobre CADA arremate que seus indicados fizerem.</p>
            ...
            <div className="pt-2">
              <Button size="lg" className={`w-full text-white font-bold text-base py-6 rounded-lg shadow-lg transition-all ${isSDB ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}`} onClick={onRegisterClick}>
        ...
        <div className="mt-12 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {benefits.map((item, index) => (
            <div key={item.text} className="flex flex-col items-center group" onMouseEnter={() => setHoveredBenefit(index)} onMouseLeave={() => setHoveredBenefit(null)}>
              <div className={`flex h-20 w-20 items-center justify-center rounded-xl bg-gray-800 border-2 mb-4 transform group-hover:scale-110 transition-all shadow-lg cursor-pointer ${isSDB ? 'border-red-500/30 group-hover:border-red-400' : 'border-green-500/30 group-hover:border-green-400'}`}>
                <item.icon className={`h-10 w-10 ${isSDB ? 'text-red-400' : 'text-green-400'}`} />
              </div>
      ...
      <AnimatePresence>
        {hoveredBenefit !== null && (
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} transition={{ duration: 0.2 }} className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-none">
            <div className={`bg-gray-800 border-2 rounded-2xl p-6 shadow-2xl max-w-sm mx-4 ${isSDB ? 'border-red-500/50' : 'border-green-500/50'}`} style={{ boxShadow: isSDB ? '0 0 60px rgba(239,68,68,0.4)' : '0 0 60px rgba(34,197,94,0.4)' }}>
              <div className="flex items-start gap-4">
                <div className={`p-3 rounded-xl flex-shrink-0 border ${isSDB ? 'bg-red-500/20 border-red-500/30' : 'bg-green-500/20 border-green-500/30'}`}>
                  {React.createElement(benefits[hoveredBenefit].icon, { className: `w-8 h-8 ${isSDB ? 'text-red-400' : 'text-green-400'}` })}
                </div>
                <div>
