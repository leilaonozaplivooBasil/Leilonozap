...
const chamadas = { busca: 0, proxy: [], upload: 0, escritas: [] };
...
ok('NENHUM botão de IA na tela (dono mandou tirar)', !(await page.evaluate(() => document.body.innerText.includes('Preencher com IA'))));
await page.evaluate(() => {
  const ta = [...document.querySelectorAll('textarea')].find((x) => x.placeholder.includes('banco de couro'));
  const setter = Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, 'value').set;
  setter.call(ta, 'BMW X6 2024, preta, rodas aro 21, bancos de couro caramelo.');
  ta.dispatchEvent(new Event('input', { bubbles: true }));
});
ok('salvou os detalhes escritos na mão', await clicaVisivel('Salvar'));
await page.waitForTimeout(1200);
t = await corpo();
