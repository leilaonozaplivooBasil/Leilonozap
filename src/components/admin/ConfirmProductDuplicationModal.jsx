...
export default function ConfirmProductDuplicationModal({
  ...
  const calcAutoPrice = () => {
    ...
    return lance > 0 ? (lance / 0.80).toFixed(2) : '';
  };
              ...
              <div className="bg-gray-800 border border-gray-700 rounded-lg p-3 space-y-3" onClick={(e) => e.stopPropagation()}>
                ...
                {catalogPrice && parseFloat(catalogPrice) > 0 && (
                  <div className="bg-gray-900/50 rounded-lg p-3 border border-gray-600 space-y-1.5">
                    ...
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-400">🛒 Loja Virtual (÷0.80):</span>
                      <span className="text-blue-400 font-bold">R$ {parseFloat(catalogPrice || 0).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-400">📊 Mercado real (÷0.64):</span>
                      <span className="text-white font-bold">R$ {(parseFloat(formData.starting_price || 0) / 0.64).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-400">⚡ Arremate Agora (= loja):</span>
                      <span className="text-green-400 font-bold">R$ {parseFloat(catalogPrice || 0).toFixed(2)}</span>
                    </div>
                  </div>
                )}
              </div>
