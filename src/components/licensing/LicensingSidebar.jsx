...
export default function LicensingSidebar({ user, shortName, activeTab, onTabChange }) {
  ...
  return (
    <aside className="hidden md:flex flex-col items-center w-24 shrink-0 bg-nz-preto-barra border-r border-black/40 py-5 gap-1 sticky top-0 h-screen overflow-y-auto">
      ...
      <nav className="flex flex-col gap-3 w-full px-2 pb-6">
        {grupos.map((grupo) => (
          <div key={grupo.title} className="flex flex-col gap-1">
            <p className="text-[8px] font-bold uppercase tracking-wider text-white/40 text-center">{grupo.title}</p>
