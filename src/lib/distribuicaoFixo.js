...
//
// 3. O MÍNIMO DIÁRIO SEGURA O VALOR: um dia com menos tarefas que o mínimo
//    paga proporcional (2 de 3 tarefas = 2/3 do dia). Sem isso, uma tarefa
//    sozinha valeria o dia inteiro, e o mínimo seria só um número na tela.
//    O que não foi preenchido fica "em aberto" — visível, não pago.
//
 ...
 */
export function distribuirDia({ fixoMes, dias = DIAS_FIXO, minimoDia = MINIMO_DIA_PADRAO, tarefas = [] } = {}) {
  ...
  const fator = Math.min(1, lista.length / minimo);
  ...
  return {
    ...
    faltam: Math.max(0, minimo - lista.length),
    fator,
    somaPesos: lista.reduce((s, t) => s + pesoValido(t.peso), 0),
    minimoDia: minimo,
  };
}
 ...
 */
export function simularNovaTarefa({ fixoMes, dias, minimoDia, tarefas = [], novaPeso = 3 } = {}) {
  const antes = distribuirDia({ fixoMes, dias, minimoDia, tarefas });
  const nova = { id: '__nova__', peso: pesoValido(novaPeso) };
  const depois = distribuirDia({ fixoMes, dias, minimoDia, tarefas: [...tarefas, nova] });
  const quedas = tarefas
    .filter((t) => t && t.id != null)
    .map((t) => ({ id: t.id, de: antes.valores[t.id] ?? 0, para: depois.valores[t.id] ?? 0 }))
    .filter((q) => Math.abs(q.de - q.para) >= 0.005);
  return {
 ...
 */
export function resumoDoCiclo({ fixoMes, dias = DIAS_FIXO, minimoDia, tarefasPorDia = {}, diasDoCiclo = [], hojeISO } = {}) {
  ...
  for (const dia of todos) {
    ...
    const d = distribuirDia({ fixoMes, dias, minimoDia, tarefas });
