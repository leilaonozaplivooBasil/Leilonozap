...
import { Loader2, Wallet, Smartphone, Package, BarChart } from 'lucide-react';
      ...
      <CardContent className="space-y-6">
        ...
        <div className={`p-4 rounded-lg ${isSaiDeBaixo ? 'bg-gray-100' : 'bg-gray-700/50'}`}>
          ...
          <div className="space-y-2 text-sm">
            <div className="flex items-start gap-2"><span className="text-green-500">📱</span><div><strong className={isSaiDeBaixo ? 'text-gray-900' : 'text-white'}>App (3%):</strong><span className={isSaiDeBaixo ? 'text-gray-600' : 'text-gray-400'}> 3% sobre cada arremate dos indicados.</span></div></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className={`p-5 rounded-xl border-2 ${isSaiDeBaixo ? 'bg-green-50 border-green-300' : 'bg-green-900/20 border-green-500/30'}`}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <div><p className={`font-bold ${isSaiDeBaixo ? 'text-gray-900' : 'text-white'}`}>📱 App (Influencer)</p><p className={`text-xs ${isSaiDeBaixo ? 'text-gray-500' : 'text-gray-500'}`}>3% por arremate</p></div>
            </div>
