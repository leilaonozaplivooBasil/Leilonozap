...
test('os slides: capa, mentalidade, abertura, leitura, treinamento, um por tópico, fechamento com as demandas', () => {
  ...
  // DIR-168.1: a lista do que precisa ser conversado ABRE a Produção; o fechamento fecha
  assert.deepEqual(s.map((x) => x.id), ['capa', 'mentalidade', 'abertura', 'leitura', 'treinamento', 'producao', 'topico-0', 'topico-1', 'fechamento']);
  assert.deepEqual(s.map((x) => x.bloco), [null, 'mentalidade', null, 'leitura', 'treinamento', 'reuniao', 'reuniao', 'reuniao', null]);
  ...
  assert.match(s[5].titulo, /Produção/);
});
