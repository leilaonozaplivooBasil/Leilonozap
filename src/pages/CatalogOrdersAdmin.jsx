...
// 🔴 PONTO 120 (21/08/2026) — o dropdown de "Status do Pedido" lá embaixo só tem
// itens no vocabulário em inglês, mas o pedido pode estar salvo em português
// (updateOrderStatus.js aceita e grava os dois — PONTO 116). Sem tradução, o
// Radix recebe um valor sem item correspondente e o campo aparece VAZIO pro
// operador.
//
// ⚠️ Uma tentativa anterior resolveu isso remapeando o valor DENTRO do
// componente Select compartilhado (src/components/ui/select.jsx), que o app
// inteiro usa: o rótulo mostrava um texto e o Radix considerava outro item
// selecionado por dentro, e reabrir o dropdown sem trocar nada podia gravar o
// status errado sozinho. Foi revertido — e NÃO deve voltar.
//
// A tradução correta é só de EXIBIÇÃO e só nesta tela: o Select sempre fala o
// vocabulário em inglês (rótulo e valor interno do Radix sempre batem), e o
// valor real do pedido (`newStatus`) só muda quando o admin escolhe algo —
// nunca sozinho.
...
export default function CatalogOrdersAdmin() {
  ...
  const handleTogglePacked = async (order, idx) => {
    ...
    try {
      ...
      // Mostra o motivo real quando o servidor manda um: "Erro ao salvar" seco
      // não diz ao operador se foi permissão, rede ou dado inválido.
  ...
  const handleSetFulfillment = async (order, value) => {
    ...
    try {
      ...
      // Sem isto o dropdown "Status do Pedido" continuava exibindo o valor
      // ANTERIOR depois de avançar a jornada — e um Salvar em seguida
      // regravava o status velho por cima do que a jornada acabou de gravar.
                  ...
                  return (
                    <div className="pt-1 border-t border-gray-600/50 mt-2 flex items-center justify-between gap-2">
                      <p className="text-sm text-green-400">
                        📦 Etiqueta comprada na Melhor Envio{envio.protocol ? ` — protocolo ${envio.protocol}` : ''}
                      </p>
                      {/* Botão de verdade, não link no meio do texto: é a ação que
                          a logística mais repete no dia. Sem label_url, o aviso
                          abaixo explica o que fazer em vez de sumir calado. */}
                      {envio.label_url ? (
                        <a
                        ...
                        <span className="shrink-0 text-[11px] text-amber-400">
                          PDF ainda não disponível — abra pelo painel da Melhor Envio
                        </span>
                      )}
                    </div>
                  );
