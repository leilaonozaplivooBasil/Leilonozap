...
export default function LuxuryAccessManager() {
  ...
  const create = async () => {
    ...
    try {
      ...
      setNewCode(""); setLabel(""); setPersonName(""); setEmail(""); setWhatsapp(""); setSingleUse(false);
      await load();
                    ...
                    <tr key={c.id} className="border-t border-gray-700/60 odd:bg-gray-800/30 hover:bg-gray-800/60 transition-colors">
                      ...
                      <td className="py-3 px-4 text-right">
                        <div className="inline-flex gap-2 flex-wrap justify-end bg-gray-700/30 border border-gray-600/40 rounded-lg p-1">
                          ...
                          {c.is_single_use && c.is_used && <Button size="sm" variant="outline" onClick={()=>resetUse(c)} className="border-gray-700 bg-gray-900 text-gray-200 hover:bg-gray-800"><RefreshCcw className="w-3.5 h-3.5 mr-1"/>Resetar</Button>}
